...
with DAG(dag_id="test_dag_simple", schedule_interval=None, default_args=default_args) as dag:
    ...
    bq_query = BigQueryOperator(
        task_id="bq_query_select_public_dataset", use_legacy_sql=False, sql=sql, depends_on_past=False, dag=dag
    )
start >> bq_query >> end
